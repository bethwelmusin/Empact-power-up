...
# class BlogsCrudUrlsTest(SimpleTestCase):
    ...
    def test_blog_detail_is_resolved(self):
        ...
        url = reverse('blog:blog-detail', args=[1]) # This the url reverse name for {all-published-blogs} url path.Requires and id add it in args
        self.assertEquals(resolve(url).func.view_class, BlogDetailApiView)

    def test_add_new_blog_is_resolved(self):
        add_blog_url = reverse('blog:add-new-blog', args=[1]) # This the url reverse name for {all-published-blogs} url path.
        get_user_blog_url = reverse('blog:add-new-blog', args=[1]) # This the url reverse name for {all-published-blogs} url path.
    ...
    def test_add_post_comment_is_resolved(self):
        comment_post_url = reverse('blog:comment-post', args=[1]) # This the url reverse name for {all-published-blogs} url path.
        get_post_comment_url = reverse('blog:get-post-comments', args=[1]) # This the url reverse name for {all-published-blogs} url path.
    ...
    def test_comment_detail_is_resolved(self):
        url = reverse('blog:comment', args=[1]) # This the url reverse name for {all-published-blogs} url path.
