...
from rest_framework import status,permissions,IsAuthenticated
    ...
    def post(self, request):
        try:
            ...
            if not mobile_number:
                return Response({
                    'status': False,
                    'message': 'Either the mobile number or the password is incorrect.'
                ...
                if not user.exists():
                    return Response({
                        'status': False,
                        'message': 'A user with this mobile number does not exists.'
            ...
            return Response({
                'status': False,
                'message': 'A user with this mobile number exists.'
            }, status=status.HTTP_404_NOT_FOUND)
