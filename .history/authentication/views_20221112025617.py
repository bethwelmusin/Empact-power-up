...
class LoginRestaurantAPIView(APIView):
    ...
    def post(self, request):
        ...
        if Restaurant.objects.filter(email=username).exists():
            payload = json.dumps({
                "email": username,
                "password": password,
            ...
            if res.status_code == 200:
                ...
                res_qs = Restaurant.objects.filter(email=email)
                res = RestaurantSerializer(res_qs.get())
